...
// ==================== ORDER OPERATIONS (for one-time payments) ====================

/**
 * Create payment order for one-time payments (e.g., paid trial)
 */
export const createPaymentOrder = async (orderData: {
  ...
  console.log(`[RAZORPAY] Creating order: ${orderData.amount / 100} ${orderData.currency}`);

  const response = await makeGatewayRequest("POST", "/orders", {
...
export const getPaymentOrder = async (orderId: string): Promise<any> => {
  return await makeGatewayRequest("GET", `/orders/${orderId}`);
};

// ==================== PAYMENT VERIFICATION ====================

/**
 * Verify payment signature for one-time orders
 */
export const verifyPaymentSignature = (
  ...
  console.log(`[RAZORPAY] Verifying payment signature for order: ${orderId}`);

  const generatedSignature = crypto
    .createHmac("sha256", GATEWAY_CONFIG.keySecret)
